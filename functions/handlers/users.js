    ...
    db.doc(`/users/${newUser.handle}`).get()
        ...
        .catch((err) => {
            ...
            if (err.code === 'auth/email-already-in-use') {
                return res.status(400).json({ email: 'Email is allready in use' });
            } else {
...
exports.uploadImage = (req, res) => {
    ...
    busboy.on('file', (fieldname, file, filename, encoding, mimetype) => {
        ...
        // my.image.png(we need to get the extension, we need to split the string by each dot)
        const imageExtension = filename.split('.')[filename.split('.').length - 1];
        // 4234523452352345.png
    ...
    busboy.on('finish', () => {
        admin
            ...
            .then(() => {
                return res.json({ message: 'Image uploaded sucesfully' });
            })
            .catch((err) => {
                console.error(err);
                return res.status(500).json({ error: err.code });
